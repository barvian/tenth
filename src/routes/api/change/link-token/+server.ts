...
export const POST: RequestHandler = async (event) => {
	...
	const { data: profile, error: profileError } = await supabaseClient
		...
		.select('change_id')
		.single()
	if (profileError || !profile?.change_id)
		throw error(500, 'Could not retrieve change_id for logged in user')

	const creds = Buffer.from(
		PUBLIC_CHANGE_KEY + ':' + SECRET_CHANGE_KEY
	).toString('base64')
	const res = await fetch(
		`https://api.getchange.io/api/v1/accounts/${profile.change_id}/link_bank_token`,
		{
			method: 'POST',
			headers: {
