...
<div class="w-full {cls}">
    <div class="group relative">
        <input bind:this={input} {maxlength} {autocomplete} {spellcheck} {name} {required} id={name} {type} {value} on:input={handleInput} on:focus on:blur placeholder={label || placeholder} class="w-full {inconspicuous ? 'bg-transparent border-none' : 'bg-white shadow border-black focus:border-orange-500 focus:shadow-orange-500/10'} peer text-lg inherit-case placeholder:normal-case {type === 'search' ? 'pl-12 pr-5 pb-2.5 pt-3 text-center placeholder:text-gray-450' : (icon || !label ? 'pl-11 py-3.5 pr-4 placeholder:text-gray-450' : 'px-4 pb-2 pt-5 placeholder-shown:pb-3.5 placeholder-shown:pt-3.5 placeholder:text-transparent')} placeholder:font-normal relative transition-all focus:ring-0 focus:ring-offset-0 {type === 'search' ? 'rounded-full' : 'rounded-2xl'} font-medium {!inconspicuous && error ? '!border-red-500 !shadow-red-500/10' : ''}" />
        ...
        {:else if label}
            <label for={name} class="absolute normal-case whitespace-nowrap pointer-events-none scale-[0.55] text-gray-450 peer-focus:text-orange-500 peer-focus:peer-placeholder-shown:text-gray-450 [&_span]:opacity-0 peer-placeholder-shown:[&_span]:opacity-100 translate-y-[calc(theme(padding.1)*2.25)] peer-placeholder-shown:translate-y-4 peer-placeholder-shown:scale-100 left-4 ml-[0.05em] top-0 origin-top-left text-lg transition-all">
    ...
    {:else if $$slots.default}
        <p in:fade|local class="text-gray-450 leading-snug mt-5"><slot /></p>
    {/if}
</div>