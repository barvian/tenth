...
<div class="flipcard relative round-2xl perspective-1400" on:animationend={() => flipping = false}>
    ...
    <div class="overlap transform-3d motion-safe:transition-transform motion-safe:duration-[600ms]" class:rotate-y-[-180deg]={flipped}>
        ...
        <div class="backface-hidden bg-white relative rounded p-10 border rotate-y-180">
            ...
            <slot name="back" />
            <button class="absolute top-6 left-5" on:click={() => flipped = false}><Arrow class="h-4 transition-transform hover:-translate-x-0.5" /></button>
