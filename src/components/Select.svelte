...
<div class="relative inline-block">
    <select class="appearance-none bg-transparent border-none opacity-0 absolute inset-0" {name} bind:value>
        <slot />
    </select>
    <label class="relative flex pl-0.5 pr-1 items-baseline text-orange-500 border-current leading-extra-tight border-b-4 border-dotted pointer-events-none" for={name}>
        {value}
